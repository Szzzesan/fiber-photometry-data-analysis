...
def single_session_analysis(animal_dir, signal_filename, arduino_filename, behav_filename):
    ...
    raw_separated = helper.de_interleave(neural_events, session_label=signal_dir[-23:-7], plot=0)
    dFF0 = helper.calculate_dFF0(raw_separated, session_label=signal_dir[-23:-7], plot=0, plot_middle_steps=0)
    dFF0.name = 'dFF0'
    ...
    pi_events = helper.data_reduction(pi_events, lick_tol=.01, head_tol=.2)
    ...
    # helper.remove_unfocused_trials(pi_trials)
    exp_events = helper.get_reward_by_trial(pi_trials, port=1)
    bg_events = helper.get_reward_by_trial(pi_trials, port=2)
    ...
    helper.export_df_to_csv(all_trial_events, event_export_dir)
    # endregion

    # endregion

    # # region Visualize with nex
    # helper.visualize_with_nex(animal_dir, non_trial_events, all_trial_events, dFF0, fps=80, session_label=signal_filename[-23:-7])
    # # endregion
