...
def single_session_analysis(animal_dir, signal_filename, arduino_filename, behav_filename, task='single_reward'):
    ...
    dFF0 = func.calculate_dFF0_Hamilos(raw_separated, session_label=signal_dir[-23:-7], plot=0,
    ...
    #         'exp_entry', 'exp_exit')
    #
    #     func.sensor_raster_plot(dFF0, pi_events, condition,
    #                             branch=b,
    #                             port='exp',
    #                             aligned_by='rewards',
    #                             sequence=0,
    ...
    intervals_df, single = func.make_intervals_df(pi_events, report_single=True)
    ...
    bg_fast = (pi_events.is_valid_trial) & (pi_events.phase == '0.8') & (pi_events.port == 2) & (
            pi_events.key == 'trial') & (pi_events.value == 1)
    bg_slow_trial_average_r = func.trial_average(pi_events, dFF0, 'right', bg_slow, [-1, 11])
    bg_fast_trial_average_r = func.trial_average(pi_events, dFF0, 'right', bg_fast, [-1, 11])
    bg_slow_trial_average_l = func.trial_average(pi_events, dFF0, 'left', bg_slow, [-1, 11])
    bg_fast_trial_average_l = func.trial_average(pi_events, dFF0, 'left', bg_fast, [-1, 11])
    func.plot_average(bg_slow_trial_average_r, bg_fast_trial_average_r, save_path=fig_export_dir,
                      save_name='dFF0_all_averaged', left_or_right='right', save=1)
    func.plot_average(bg_slow_trial_average_l, bg_fast_trial_average_l, save_path=fig_export_dir,
    ...
    # func.slow_timescale_analysis(dFF0, pi_events, timescale_in_sec=30)
    # endregion
