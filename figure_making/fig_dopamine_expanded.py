...
def plot_heatmap_and_mean_traces(time_vector, category_codes, cat_labels, heatmap_matrix, palette='Reds_r', split_cat=0,
                                 legend_title='Time in Port', animal=None, hemi=None, session_name=None, axes=None):
    """
    ...
    if axes is None:
        ...
        suptitle = f'{animal}{hemi}: {session_name}'
        return_handle = True
    else:
        fig = None
        return_handle = False
...
def figc_example_session_heatmap_split_by_NRI(zscore, zscore_branch, reward_df, animal=None, hemi=None, session=None,
                                              axes=None):
    time_vec, cat_codes, cat_labels, heatmap_mat = resample_data_for_heatmap(
        zscore, zscore_branch, reward_df,
        cutoff_pre_reward=-0.5,
    ...
    plot_heatmap_and_mean_traces(time_vec, cat_codes, cat_labels, heatmap_mat, palette='Reds_r', split_cat=0,
                                 legend_title=f'Reward Time\nfrom Entry (s)', animal=animal, hemi=hemi,
                                 session_name=session, axes=axes)
...
def figd_example_session_heatmap_split_by_block(zscore, zscore_branch, reward_df, animal=None, hemi=None, session=None,
                                                axes=None):
    time_vec, cat_codes, cat_labels, heatmap_mat = resample_data_for_heatmap(
        zscore, zscore_branch, reward_df,
        cutoff_pre_reward=-0.5,
    ...
    plot_heatmap_and_mean_traces(time_vec, cat_codes, cat_labels, heatmap_mat, palette='Set2', split_cat=1,
                                 legend_title=f'Context\nReward Rate', animal=animal, hemi=hemi, session_name=session,
                                 axes=axes)
...
def figg_LMEM_coefficients_v3(master_df, axes=None):
    ...
    random_slopes = (
        "1 + "  # Random Intercept for Animal
        "logNRI_std + "  # Random Slope for NRI
        "logIRI_std + "  # Random Slope for IRI
        "C(block, Treatment(reference='0.8'))"  # Random Slope for Block
    )
    ...
    axes.set_xlabel('Regressor', labelpad=-5)
    axes.tick_params(axis='x', pad=0)
    axes.set_xticklabels(axes.get_xticklabels(), rotation=15, ha='right')  # Increased rotation for longer labels
    axes.spines['right'].set_visible(False)
    axes.spines['top'].set_visible(False)
...
def setup_axes():
    fig_size = (12, 14)  # (width, height) in inches
    ...
    row_3_margins = [3]
    row_4_margins = [8, 10, 8]
    col_1_margins = [10, 10, 10]
    col_2_margins = [9, 9]
    ...
    col_2_splits = [int((rows - np.sum(col_2_margins) - col_1_splits[0] - col_1_margins[0] - col_1_splits[-1] -
                         col_1_margins[-1]) * h / sum(col_2)) for h in
                    col_2]
    ...
    col_2_splits[1:] += col_1_splits[-3] - col_2_splits[-1]
    ...
    axes = [
        ...
        fig.add_subplot(gs[col_1_splits[5]:col_1_splits[6], :row_4_splits[0]]),
        fig.add_subplot(gs[col_1_splits[5]:col_1_splits[6], row_4_splits[1]:row_4_splits[2]]),
        fig.add_subplot(gs[col_1_splits[5]:col_1_splits[6], row_4_splits[3]:row_4_splits[4]]),
        fig.add_subplot(gs[col_1_splits[5]:col_1_splits[6], row_4_splits[5]:row_4_splits[6]]),
    ]
    ...
    axes_to_letter = [0, 2, 3, 7, 11, 12, 14, 15, 16, 17, 18]
...
if __name__ == '__main__':
    ...
    #                                                file_format='parquet')
    # figc_example_session_heatmap_split_by_NRI(zscore_heatmap, 'green_right', reward_df, axes=None)
